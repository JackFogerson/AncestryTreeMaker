...
# Attempt to load geopy for location geocoding
try:
...
class AncestryApp:
    ...
    def build_timeline_steps(self):
        ...
        for year in range(self.min_year, self.max_year + 1):
            max_k = 0
            for node in self.tree.values():
                v_locs = [loc for loc in sorted(node.locations, key=lambda x: x['year'])
                          if loc.get('lat') is not None and loc.get('lon') is not None]
                year_locs = [loc for loc in v_locs if loc['year'] == year]
                if len(year_locs) > max_k:
                    max_k = len(year_locs)

            if max_k <= 1:
                self.timeline_steps.append((year, 1))
            else:
                for k in range(1, max_k + 1):
                    self.timeline_steps.append((year, k))
    ...
    def open_profile_dialog(self, person_name):
        ...
        def render_checkboxes():
            ...
            for idx, eth in enumerate(self.ethnicity_options):
                ...
                lbl = tk.Label(cb_container, bg=self.ethnicity_colors.get(eth, "#cbd5e1"), width=3, relief=tk.RAISED,
                               cursor="hand2")
